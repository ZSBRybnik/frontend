...
(async () => {
  ...
  if (docker) {
    ...
    const developmentServer = $`yarn run remove-build && cross-env tsc --project tsconfig.noemit.json && cross-env NODE_OPTIONS="--max-old-space-size=8192" TS_NODE_PROJECT=tsconfig.node.json webpack serve --config webpack.config.ts --env target=${target} --mode development`;
